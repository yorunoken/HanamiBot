...
export default {
  ...
  embeds: {
    ...
    whatif: {
      count: `a`,
      plural: `s`,
      title: (username: string, length: strOrNum, pp: strOrNum) => `{username} gets {length} new {pp}pp play{s}`,
      ...
      description: (length: strOrNum, pp: strOrNum, username: string, position: strOrNum, newPp: strOrNum, diffPp: strOrNum, rank: strOrNum, rankDiff: strOrNum) => `{length} new {pp}pp play{s} would be {username}'s #{position} top play.\nIt would increase their total pp to {newPp}, and by {diffPp} and increase their rank to #{rank} (+{rankDiff}).`,
    },
    pp: {
  ...
  fails: {
    ...
    provideValidPage: `Please provide a valid page (between 1 and {maxValue})`,
