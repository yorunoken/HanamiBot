...
exports.run = async (client, message, args, prefix, EmbedBuilder) => {
	...
	async function GetManga(manga) {
		try {
			...
			const embed = new EmbedBuilder()
				...
				.setFields({ name: `**Manga Description:**`, value: `${manga.localizedDescription.localString}`, inline: false }, { name: `**Content Rating:**`, value: `${manga.contentRating}`, inline: true }, { name: `**Manga Rating:**`, value: `${stats.rating.average}`, inline: true }, { name: `**Status:**`, value: `${manga.status}`, inline: true }, { name: `**Tags:**`, value: `${genres.join(", ")}`, inline: true }, { name: `\u200B`, value: `\u200B`, inline: true }, { name: `**Available Languages:**`, value: `${manga.availableTranslatedLanguages.join(", ")}`, inline: true }, { name: `**First Chapter:**`, value: `${first_chapter_index}`, inline: true }, { name: `**Latest Chapter:**`, value: `${latest_chapter_index}`, inline: true })
	...
	MFA.login(process.env.mangauser, process.env.mangapass, "osuBeatmapCache/").then(async () => {
		try {
