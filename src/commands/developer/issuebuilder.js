	...
	try {
		if (message.reference.messageId) {
			message.channel.messages.fetch(message.reference.messageId).then(async Content => {
				...
				const confirm = new EmbedBuilder().setTitle("Issue builder v2").setThumbnail(`${user.displayAvatarURL()}?size=1024`).setDescription(`Preview of issue:\n\n**${Title}**\n${Content.content}`).setFields({ name: `Added labels:`, value: addedlabels })

				const row = new ActionRowBuilder().addComponents(
					new ButtonBuilder().setLabel("Bug").setCustomId("bug").setStyle(ButtonStyle.Secondary),

					new ButtonBuilder().setLabel("Suggestion").setCustomId("suggestion").setStyle(ButtonStyle.Secondary),

					new ButtonBuilder().setLabel("High priority").setCustomId("priority").setStyle(ButtonStyle.Secondary),
				)

				const row2 = new ActionRowBuilder().addComponents(
					new ButtonBuilder().setLabel("New command").setCustomId("new").setStyle(ButtonStyle.Secondary),

					new ButtonBuilder().setLabel("First issue").setCustomId("first").setStyle(ButtonStyle.Secondary),
				)
					...
					collector.on("collect", async i => {
						try {
							...
							if (i.customId == "confirm") {
								...
								const embed = new EmbedBuilder().setTitle("Issue builder v2").setColor("Purple").setDescription(`Successfully created Issue #${issuebuilder.data.number}\n[Click here to go to issue](https://github.com/YoruNoKen/miaosu/issues/${issuebuilder.data.number})`).setFields({ name: `Added Labels:`, value: labelNames }, { name: `Assigned:`, value: `YoruNoKen` })
