...
exports.run = async (client, message, args, prefix) => {
	...
	if (server == "saber") {
		var repsonse = await fetch(`http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${process.env.STEAM_API_KEY}&vanityurl=${username}`, { method: "GET" });
		...
		fs.readFile("./user-data.json", (error, data) => {
			...
			fs.writeFile("./user-data.json", JSON.stringify(userData, null, 2), error => {
				if (error) {
					...
					message.reply(`Set Beatsaber id to **${user_id}**`);
				}
			});
		});
	}
