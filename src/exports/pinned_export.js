...
async function GetPinned(value, user, mode, RuleSetId, pageNumber) {
  ...
  if (pin[one]) {
    pin_one = `${await Pinget(pin[one], one)}\n`
  }

  pin_two = ""
  if (pin[two]) {
    pin_two = `${await Pinget(pin[two], two)}\n`
  }

  pin_three = ""
  if (pin[three]) {
    pin_three = `${await Pinget(pin[three], three)}\n`
  }

  pin_four = ""
  if (pin[four]) {
    pin_four = `${await Pinget(pin[four], four)}\n`
  }

  pin_five = ""
  if (pin[five]) {
    pin_five = `${await Pinget(pin[five], five)}`
  }
