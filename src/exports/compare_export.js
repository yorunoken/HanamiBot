...
async function CompareEmbed(mapinfo, beatmapId, user, ModeString, value, pagenum) {
	...
	let pageCount = ``
	pageCount = `**Page:** \`${one + 1}/${PageCounter}\``
	...
	async function GetCompareList(score, num) {
		...
		//normal pp
		let Attrs = calc.n100(score.statistics.count_100).n300(score.statistics.count_300).n50(score.statistics.count_50).nMisses(Number(score.statistics.count_miss)).combo(score.max_combo).nGeki(score.statistics.count_geki).nKatu(score.statistics.count_katu).mods(modsID).performance(map)
		...
		// score set at
		let ScoreSetAt = new Date(score.created_at).getTime() / 1000
		...
		const row_one = `**${num + 1}.**${grade} \`+${Mods}\` **[${maxAttrs.difficulty.stars.toFixed(2)}★]** ∙ ${score.score.toLocaleString()} ∙ (${(score.accuracy * 100).toFixed(2)}%)\n`
		const row_two = `▹${pps}\n`
		const row_three = `▹[**${score.max_combo}**x/${FCAttrs.difficulty.maxCombo}x] ∙ ${AccValues} <t:${ScoreSetAt}:R>`
