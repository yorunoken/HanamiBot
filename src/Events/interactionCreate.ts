...
export default class InteractionCreateEvent extends BaseEvent {
    ...
    public async execute(interaction: Interaction): Promise<void> {
        ...
        if (interaction.isModalSubmit()) {
            ...
            const index = sillyOptions.embedOptions.index !== undefined ? Number(interaction.fields.getTextInputValue("index")) : null;
            const page = sillyOptions.buttonHandler === "handleTopsButtons" && sillyOptions.embedOptions.index === undefined ? Number(interaction.fields.getTextInputValue("page")) : null;
            const playsLength = sillyOptions.embedOptions.plays.length;

            if ((index ?? page ?? 0) > (
                sillyOptions.buttonHandler === "handleRecentButtons"
                    ? playsLength
                    : sillyOptions.embedOptions.index !== undefined
                        ? playsLength
                        : Math.ceil(playsLength / 5)
            )
                        || (index ?? page ?? 0) < 0)
                return;

            if (index) sillyOptions.embedOptions.index = Number(index) - 1;
            if (page)sillyOptions.embedOptions.page = Number(page) - 1;
        ...
        if (interaction.type === InteractionType.MessageComponent && interaction.isButton()) {
            ...
            if (interaction.user.id !== sillyOptions.initializer.id) {
                await interaction.reply({ ephemeral: true, content: locale.fails.userButtonNotAllowed });
                return;
            }
            ...
            if (interaction.customId === "indexbtn") {
                ...
                const favoriteColorInput = new TextInputBuilder()
                    .setCustomId(sillyOptions.buttonHandler === "handleRecentButtons" ? "index" : sillyOptions.embedOptions.index !== undefined ? "index" : "page")
                    .setLabel(locale.modals.valueInsert(sillyOptions.buttonHandler === "handleRecentButtons" ?
                        playsLength
                        : sillyOptions.embedOptions.index !== undefined
                            ? playsLength
                            : Math.ceil(playsLength / 5)))
        ...
        if (interaction.type === InteractionType.ApplicationCommand) {
            try {
                ...
                command.run({ client: this.client, interaction, db, locale }).catch(async (error: Error) => {
                    ...
                    const messageAuthor = `<@${interaction.user.id}> (${interaction.user.username})`;
                    const channelLink = `https://discord.com/channels/${interaction.guildId}/${interaction.channelId}`;
                    const messageContent = `[/${cmd}](${channelLink}/${interaction.id})`;
                    const guildName = `[${interaction.guild?.name}](${channelLink})`;

                    const embed = new EmbedBuilder()
                        .setTitle(`Runtime error on command: ${cmd}`)
                        .addFields(
                ...
                if (cmd) {
                    ...
                    insertData({ table: "commands", id: cmd, data: [ { name: "count", value: doc ? doc.count + 1 : 1 } ] });
                }
            } catch (e) {
                console.error(e);
                await interaction.reply({ content: locale.fails.interactionError, ephemeral: true });
            }
            return;
        }
    }
