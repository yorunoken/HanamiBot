...
export async function getScore({ plays, index, mode, _isTops, isCompare, perfDetails, beatmap, file }: { plays: ScoreResponse[]; index: number; mode: osuModes; _isTops?: boolean; isCompare?: boolean; perfDetails?: any; beatmap?: BeatmapResponse; file?: string }): Promise<ScoreInfo> {
  ...
  return {
    ...
    ifFcValue: (performance.curPerf as any).effectiveMissCount > 0
      ? `If FC: **${performance.fcPerf.pp.toFixed(2)}**pp for **${
        tools
          .accuracy(
            {
...
export async function getBeatmap(map: BeatmapResponse, valueOptions: { mods: string[]; accuracy?: number; ar?: number; od?: number; cs?: number }, file: string): Promise<BeatmapInfo> {
  ...
  return {
    ...
    updatedAt: `${map.status === "ranked" ? "Ranked at" : map.status === "loved" ? "Loved at" : map.status === "qualified" ? "Qualified at" : "Last updated at"} ${
      new Date(map.last_updated).toLocaleDateString("en-US", {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
